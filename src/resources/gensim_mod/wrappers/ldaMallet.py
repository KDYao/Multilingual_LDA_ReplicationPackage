...
def check_output(stdout=subprocess.PIPE, stderr=subprocess.PIPE, *popenargs, **kwargs):
    ...
    try:
        ...
        process = subprocess.Popen(stdout=stdout, stderr=stderr, *popenargs, **kwargs)
        ...
        if retcode:
            ...
            error.output = output
            raise error
        return output.decode('utf-8') + unused_err.decode('utf-8')
...
class LdaMallet(utils.SaveLoad, basemodel.BaseTopicModel):
    ...
    def __init__(self, mallet_path, corpus=None, num_topics=100, alpha=50, beta=0.01, id2word=None, workers=4, prefix=None,
        ...
        beta: float, optional (we customized)
            Beta parameter of LDA
        ...
        if self.id2word is None:
            if corpus:
                ...
                logger.warning("No word id and corpus provided; You should only see this when running LDA3."
                               "Parameter self.num_terms will be disabled (-1) now.")
                self.num_terms = -1
        else:
    ...
    def convert_input(self, corpus, infer=False, serialize_corpus=True):
        ...
        self.output += str(check_output(args=cmd, shell=True))
    ...
    @classmethod
    def train_multilingual(cls, **kwargs):
        """Train Multi-Lingual Mallet LDA.

        Parameters
        ----------
        corpus : tuple of log and text corpus; each corpus is an iterable of iterable of (int, int)
            Corpus in BoW format

        """

        models = kwargs.pop('models')
        corpuses = kwargs.pop('corpus')
        cls_settings = cls(**kwargs)

        for mod, corpus in zip(models, corpuses):
            mod.convert_input(corpus, infer=False)
        # log_mod.convert_input(log_corpus, infer=False)
        # text_mod.convert_input(text_corpus, infer=False)

        # --num-threads (workers) are not supported in multilingual mallet
        cmd = models[0].mallet_path + ' run cc.mallet.topics.PolylingualTopicModel --language-inputs %s ' \
                                    '--num-topics %s --alpha %s --beta %s --optimize-interval %s ' \
                                    '--output-state %s --output-doc-topics %s --output-topic-keys %s ' \
                                    '--num-iterations %s --inferencer-filename %s --doc-topics-threshold %s  ' \
                                    '--random-seed %s'

        cmd = cmd % (
            ' '.join([mod.fcorpusmallet() for mod in models]), cls_settings.num_topics, cls_settings.alpha,
            cls_settings.beta, cls_settings.optimize_interval, cls_settings.fstate(),
            cls_settings.fdoctopics(), cls_settings.ftopickeys(), cls_settings.iterations,
            cls_settings.finferencer(), cls_settings.topic_threshold, str(cls_settings.random_seed)
        )
        # NOTE "--keep-sequence-bigrams" / "--use-ngrams true" poorer results + runs out of memory
        logger.info("training MALLET LDA with %s", cmd)

        cls_settings.output += str(check_output(args=cmd, shell=True))

        # The following function gets log/text topics separately
        #word_topics_log, word_topics_text = cls_settings.load_multilingual_word_topics_seperate(models=(log_mod, text_mod))
        cls_settings.load_multilingual_word_topics(models=models)
    ...
    def load_multilingual_word_topics_seperate(self, models):
        """Load words X topics matrix from :meth:`gensim.models.wrappers.ldamallet.LdaMallet.fstate` file.
        Returns
        -------
        numpy.ndarray
        ...
        # Load word2id from both log/text
        (word_topics_log, word2id_log), (word_topics_text, word2id_text) = map(self.get_word2id, models)
        ...
        with utils.open(self.fstate(), 'rb') as fin:
            ...
            for lineno, line in enumerate(fin):
                line = utils.to_unicode(line)
                doc, language, pos, typeindex, token, topic = line.split(" ")
                # If log
                if language == '0':
                    if token not in word2id_log:
                        continue
                    tokenid = word2id_log[token]
                    word_topics_log[int(topic), tokenid] += 1.0
                # If text
                elif language == '1':
                    if token not in word2id_text:
                        continue
                    tokenid = word2id_text[token]
                    word_topics_text[int(topic), tokenid] += 1.0
                else:
                    logger.warning('Unknown language type at line: %d', lineno)
        return word_topics_log, word_topics_text

    def load_multilingual_word_topics(self, models):
        """Load words X topics matrix from :meth:`gensim.models.wrappers.ldamallet.LdaMallet.fstate` file.
        Returns
        -------
        numpy.ndarray
        ...
        # log_model, text_model = models
        #
        # id2word_log = log_model.id2word
        # id2word_text = text_model.id2word
        #
        # word2id = id2word_log.token2id
        # num_log_words = log_model.num_terms
        # for k, v in id2word_text.token2id.items():
        #     word2id[k] = v + num_log_words
        ...
        for mod in models:
            ...
            for k, v in id2word.token2id.items():
                word2id[k] = v + prev_count
            prev_count += len(id2word)


        # update number of terms
        self.num_terms = 1 + max(word2id.values())
        word_topics = numpy.zeros((self.num_topics, self.num_terms), dtype=numpy.float64)
        ...
        # Assign id2word, which will be further needed when feeding mallet lda to gensim
        # We currenlty skip setting other params since they are not needed in future code snippets
        self.id2word = Dictionary()
        self.id2word.token2id = word2id

    def get_word2id(self, obj):
        """
        Convert token2id/id2word to word2id
        Parameters
        ----------
        obj: Object of LDAMallet

        Returns
        -------
        word2id
        """
        word_topics = numpy.zeros((self.num_topics, obj.num_terms), dtype=numpy.float64)
        if hasattr(obj.id2word, 'token2id'):
            word2id = obj.id2word.token2id
        else:
            word2id = revdict(obj.id2word)
        return word_topics, word2id
    ...
    def get_perplexity(self):
        """
        Return the final perplexity of LDA model
        LL is the model's log-likelihood divided by the total number of tokens,
        this is a measure of how likely the data are given the model.
        Increasing values mean the model is improving.
        An example of the format recorded in output: <10> LL/token: -8.84549
        We grep all of these LL/token values and transform the log likelihood to perplexity: perp = perplexity = 2^(-LL/token)
        Returns: perp
        -------
        """
        # Read output
        ll_re = re.compile("<\d+>\sLL/token:\s(.*?)\n")
        perplexities = [numpy.exp2(-1 * float(x)) for x in ll_re.findall(string=self.output)]
...
def malletmodel2ldamodel(mallet_model, gamma_threshold=0.001, iterations=50):
    ...
    model_gensim = LdaModel(
        ...
        alpha=mallet_model.alpha, eta=mallet_model.beta,
